...
public class Student {
    ...
    public boolean meetPrerequisites(CourseToTake course, ArrayList<Record> recordsStudent) {
        ArrayList<CoursePrerequesite> coursesP = course.getCoursesPrerequisite();
        int counter = coursesP.size();
        for (int j = 0; j < coursesP.size(); j++) {
            for (int i = 0; i < recordsStudent.size(); i++) {
                if (coursesP.get(j).getIdCourse().equalsIgnoreCase(recordsStudent.get(i).getIdCourse())) {
//                    coursesP.remove(j);
                    counter--;
                }
            }
